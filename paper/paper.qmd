...
ggplot(state_summary, aes(x = reorder(state, -avg_pct), y = avg_pct)) +
  ...
  theme(axis.text.y = element_text(size = 8, hjust = 1, lineheight = 10))
>>>>>>> Stashed changes
```
...
# Create a line graph
ggplot(data, aes(x = start_date, y = pct, color = state)) +
  geom_line() +
  labs(title = "Poll Results Over Time", x = "Date", y = "Percentage")
```
